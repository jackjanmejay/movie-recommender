...
Uses curated sample data for reliable, recognizable recommendations
"""
...
class EnhancedRecommender:
    ...
    def initialize(self, min_ratings=5):
        """
        Load all data with curated sample content
        
        Parameters:
        - min_ratings: Minimum ratings required for a title to be included
        """
        ...
        # Load curated sample data (reliable, recognizable TV shows & documentaries)
    ...
    def recommend(self, content_type=None, genre=None, n=5, min_ratings=10):
        ...
        - content_type: 'Movie', 'TV Series', 'Documentary', or None (all)
    ...
    def recommend_by_decade(self, decade, n=10, min_ratings=10):
        """Get best movies from a specific decade (e.g., 1990 = 1990s)"""
        if self.content is None:
            print("❌ Please run initialize() first")
            return []
    ...
    def search(self, keyword, n=10, min_ratings=10):
        ...
        return list(results['display_title'])
    ...
    def get_statistics(self):
        ...
        # Year range (filter out None values)
    ...
    def get_tv_series_by_year(self, year, n=10, min_ratings=1000):
        """Get best TV series from a specific year"""
        if self.content is None:
            print("❌ Please run initialize() first")
            return []
        
        filtered = self.content[
            (self.content['type'] == 'TV Series') &
            (self.content['rating_count'] >= min_ratings) &
            (self.content['year'] == year)
        ].sort_values('avg_rating', ascending=False).head(n)
        
        if len(filtered) == 0:
            print(f"\n⚠️ No TV series found from {year} with at least {min_ratings} ratings")
            return []
        
        print(f"\n📺 TOP TV SERIES FROM {year}")
        print("=" * 60)
        
        for idx, (_, row) in enumerate(filtered.iterrows(), 1):
            print(f"{idx}. {row['display_title']}")
            print(f"   ⭐ {row['avg_rating']:.1f}/10 | 📊 {int(row['rating_count']):,} ratings")
        
        return list(filtered['display_title'])
...
def main():
    ...
    # Initialize with curated sample data
    recommender.initialize(min_ratings=5)
    
    # Show statistics
    recommender.get_statistics()
    
    # Example 1: Top movies (with at least 50 ratings for quality)
    print("\n" + "="*70)
    recommender.recommend(content_type='Movie', n=5, min_ratings=50)
    
    # Example 2: Top TV series
    print("\n" + "="*70)
    recommender.recommend(content_type='TV Series', n=5, min_ratings=1000)
    
    # Example 3: Top documentaries
    print("\n" + "="*70)
    recommender.recommend(content_type='Documentary', n=5, min_ratings=1000)
    
    # Example 4: Comedy movies
    print("\n" + "="*70)
    recommender.recommend(content_type='Movie', genre='Comedy', n=5, min_ratings=50)
    
    # Example 5: Compare content types
    recommender.compare_types(min_ratings=50)
    
    # Example 6: Search
    recommender.search('Matrix', n=5, min_ratings=50)
    
    # Example 7: Best movies from 1994
    recommender.recommend_by_year(1994, content_type='Movie', n=5, min_ratings=50)
    
    # Example 8: Best content from 2019
    recommender.recommend_by_year(2019, n=5, min_ratings=1000)
    
    # Example 9: Best movies from 1990s
    recommender.recommend_by_decade(1990, n=5, min_ratings=50)
    
    # Example 10: Best TV series from 2019
    recommender.get_tv_series_by_year(2019, n=5, min_ratings=1000)
